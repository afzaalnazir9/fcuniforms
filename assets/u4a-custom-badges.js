...
jQuery(function ($) {
  ...
  $(document).on('click', '.select-seal-opener', function () {
    // $(document).find('input#backing-attachment').val($(this).data('backing-attachment'));
    document.body.classList.add('overflow-hidden');
    document.body.dispatchEvent(new CustomEvent('modalOpen'));
    $('#PopupModal-select-seal').attr('open', 'true');
    $('.t4s-section-header.shopify-section-header-sticky').css({ "position": "unset" })
  });

  $(document).on('click', '#ModalClose-select-seal', function () {
    $('body').removeClass('overflow-hidden')
    document.body.dispatchEvent(new CustomEvent('modalClosed'));
    $('#PopupModal-select-seal').removeAttr('open');
    $('.t4s-section-header.shopify-section-header-sticky').css({ "position": "" })

  });


  $(document).on('click', '.select-backing-attachment-opener', function () {
    document.body.classList.add('overflow-hidden');
    document.body.dispatchEvent(new CustomEvent('modalOpen'));
    $('#PopupModal-backing-attachment').attr('open', 'true');
    $('.t4s-section-header.shopify-section-header-sticky').css({ "position": "unset" })
  });

  $(document).on('click', '#ModalClose-select-backing-attachment', function () {
    $('body').removeClass('overflow-hidden')
    document.body.dispatchEvent(new CustomEvent('modalClosed'));
  ...
  $('button.product-form__submit').on('click', function (event) {
    ...
    $(document).ready(function () {
      // Find the price element and get its data-price attribute
      const priceElement = $('.badge-price .price-per-quantity');
      // Convert the price to a number by removing any non-numeric characters and parsing it as a float
      const totalPrice = parseFloat(priceElement.data('price').replace(/[^0-9.-]+/g, ""));

      // Assuming 'product' is an object that you want to update
      // Assign the numeric price to product.price
  ...
  $(document).on('click', '#getProductionTemplate', function (event) {
    ...
    var quantityInput = document.querySelector('input[name="quantity"]');
    var quant = parseInt(quantityInput.value, 10);
...
function calculatePricing(quantity) {
  const strPrice = $(document).find('.badge-price .price-per-quantity span').text().trim()
  ...
  if (quantity <= 2) {
    perPiecePrice = parseFloat($(document).find('.badge-price .price-per-quantity span').text().trim())
  } else if (quantity <= 5) {
